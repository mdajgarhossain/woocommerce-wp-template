	...
	class WPTT_WebFont_Loader {
		...
		public function get_remote_files_from_css() {
			...
			foreach ( $font_faces as $font_face ) {
				...
				$ecommerce_online_store_style = explode( '}', $font_face )[0];

				// Sanity check.
				if ( false === strpos( $ecommerce_online_store_style, 'font-family' ) ) {
					continue;
				}

				// Get an array of our font-families.
				preg_match_all( '/font-family.*?\;/', $ecommerce_online_store_style, $matched_font_families );

				// Get an array of our font-files.
				preg_match_all( '/url\(.*?\)/i', $ecommerce_online_store_style, $matched_font_files );
